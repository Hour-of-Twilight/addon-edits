...
local function BagnonTableContains(tbl, value)
	if type(tbl) ~= "table" then return false end
	for _, v in pairs(tbl) do if v == value then return true end end
	return false
end

local function BagnonComputeUpgradeInfo(itemLinkOrIdentifier)
	...
	local tracked = BagnonTableContains(_G.CharIlevelItemSlots, numericSlotId)
...
local function BagnonEnsureUpgradeArrow(self)
	if self.upgradeArrow then return end
	local arrow = self:CreateTexture(nil, 'OVERLAY')
	arrow:SetTexture("Interface\\CraftingFrame\\UpgradeArrow")
	arrow:SetSize(13, 15)
	arrow:SetPoint('TOPLEFT', self, 'TOPLEFT', 2, -2)
	arrow:SetVertexColor(0, 1, 0, 0.9)
	arrow:Hide()
	self.upgradeArrow = arrow
end

local function BagnonUpdateUpgradeArrow(self, itemIdentifier)
	BagnonEnsureUpgradeArrow(self)
	local isUpgrade, _, _, _, _, tracked = BagnonComputeUpgradeInfo(itemIdentifier)
	if isUpgrade and tracked then self.upgradeArrow:Show() else self.upgradeArrow:Hide() end
end

local function BagnonAppendUpgradeTooltip(self, itemIdentifier)
	if not GameTooltip:IsOwned(self) then return end
	local isUpgrade, diff, color = BagnonComputeUpgradeInfo(itemIdentifier)
	...
	if self:IsCached() then
		...
		if self:IsBank() then
			if self:GetItem() then
				self:AnchorTooltip()
				local invID = BankButtonIDToInvSlotID(self:GetID())
				GameTooltip:SetInventoryItem('player', invID)
				GameTooltip:Show()
				CursorUpdate(self)
				BagnonAppendUpgradeTooltip(self, invID and GetInventoryItemLink('player', invID) or nil)
			end
		else
			ContainerFrameItemButton_OnEnter(self)
			BagnonAppendUpgradeTooltip(self, GetContainerItemLink(self:GetBag(), self:GetID()))
		end
	end
end
...
function ItemSlot:Update()
	...
	if self:IsBank() then
		local invID = BankButtonIDToInvSlotID(self:GetID())
		if invID then itemIdentifier = GetInventoryItemLink('player', invID) end
	elseif self.GetBag then
		local bag = self:GetBag()
		local slot = self:GetID()
		if bag and slot then itemIdentifier = GetContainerItemLink(bag, slot) end
	end

	BagnonUpdateUpgradeArrow(self, itemIdentifier)

	if GameTooltip:IsOwned(self) then
		self:UpdateTooltip()
		BagnonAppendUpgradeTooltip(self, itemIdentifier)
	end
end
