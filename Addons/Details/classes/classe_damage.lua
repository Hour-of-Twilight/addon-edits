...
--[[exported]]	function _detalhes:GetBarColor (actor)
				actor = actor or self
				local player_sub = HoT_Tools:check_subclass_by_aura(actor.displayName)
				if (actor.monster) then
					return _unpack (_detalhes.class_colors.ENEMY)

				elseif (actor.owner) then
					return _unpack (_detalhes.class_colors [player_sub or "UNKNOW"])
				...
				--	return 0.94117, 0.1, 0.1, 1
					...
					if (not is_player_class [actor.classe] and actor.flag_original and _bit_band (actor.flag_original, 0x00000020) ~= 0) then --> neutral
						...
						return _unpack (HoT_Tools.CLASS_COLORS[player_sub] or _detalhes.class_colors["UNKNOW"])
					end
...
function atributo_damage:ToolTip_DamageDone (instancia, numero, barra, keydown)
	...
	if (owner and player_sub) then
		r, g, b = unpack (HoT_Tools.CLASS_COLORS[player_sub])
	else
		if (not HoT_Tools.CLASS_COLORS[player_sub]) then
			return print ("Details!: error class not found:", player_sub, "for", self.nome)
		end
		r, g, b = unpack (HoT_Tools.CLASS_COLORS[player_sub])
	end
...
function atributo_damage:ToolTip_Enemies (instancia, numero, barra, keydown)
	...
	for o = 1, i-1 do
		...
		local player_sub = HoT_Tools.check_subclass_by_aura(player_name)
		...
		local r, g, b = unpack (HoT_Tools.CLASS_COLORS[player_sub])
