...
local function HoTBagTableContains(tbl, value)
	if type(tbl) ~= "table" then return false end
	for _, v in pairs(tbl) do if v == value then return true end end
	return false
end

local function HoTBagComputeUpgradeInfo(itemLinkOrIdentifier)
	...
	local _, actualLink, _, currentItemLevel, _, _, _, _, itemEquipLoc = GetItemInfo(itemLinkOrIdentifier)
	if not currentItemLevel or currentItemLevel <= 0 or not itemEquipLoc or itemEquipLoc == "" then return false end
	local numericSlotId = ConvertEquipSlotStringToNumericId(itemEquipLoc)
	if not numericSlotId then return false end

	local equipped = GetMaxItemLevelForSlot(numericSlotId)
	if equipped == 0 then equipped = GetCurrentItemLevelForSlot(numericSlotId) end
	local tracked = HoTBagTableContains(_G.CharIlevelItemSlots, numericSlotId)
	local isUpgrade = currentItemLevel > equipped and (((equipped or 0) >= 0 and tracked) or ((not tracked) and (equipped or 0) > 0))
	if not isUpgrade then return false end

	local diff = currentItemLevel - equipped
	local color
	if diff >= 15 then color = "|cFFFF6600"
	elseif diff >= 8 then color = "|cFF00FF00"
	elseif diff >= 4 then color = "|cFF40FF40"
	else color = "|cFF80FF80" end

	return true, diff, color, currentItemLevel, equipped, tracked, actualLink or itemLinkOrIdentifier
end

local function HoTBagEnsureUpgradeArrow(slot)
	if slot.upgradeArrow then return end
	local arrow = slot:CreateTexture(nil, "OVERLAY")
	arrow:SetTexture("Interface\\CraftingFrame\\UpgradeArrow")
	arrow:SetSize(13, 15)
	arrow:SetPoint("TOPLEFT", slot, "TOPLEFT", 2, -2)
	arrow:SetVertexColor(0, 1, 0, 0.9)
	arrow:Hide()
	slot.upgradeArrow = arrow
end

local function HoTBagUpdateUpgradeArrow(slot, itemIdentifier)
	HoTBagEnsureUpgradeArrow(slot)
	local isUpgrade, _, _, _, _, tracked = HoTBagComputeUpgradeInfo(itemIdentifier)
	if isUpgrade and tracked then slot.upgradeArrow:Show() else slot.upgradeArrow:Hide() end
end

local function HoTBagAppendUpgradeTooltip(slot, itemIdentifier)
	if not GameTooltip:IsOwned(slot) then return end
	local isUpgrade, diff, color = HoTBagComputeUpgradeInfo(itemIdentifier)
	...
	if tipName then
		for i = 2, 20 do
			...
			if text and text:find("Item Level Upgrade", 1, true) then return end
		end
	end
	GameTooltip:AddLine(" ")
	GameTooltip:AddLine("|TInterface\\CraftingFrame\\UpgradeArrow:16:16:0:0|t |cFF00FF00Item Level Upgrade|r", 1, 1, 1)
	GameTooltip:AddLine("|cFFFFFFFF" .. color .. "+" .. diff .. "|r item levels over max equipped.|r", 0.8, 0.8, 0.8)
	GameTooltip:Show()
end
...
function B:UpdateSlot(frame, bagID, slotID)
	...
	local hotItemIdentifier
	if slot.GetInventorySlot then local inv = slot:GetInventorySlot(); if inv then hotItemIdentifier = GetInventoryItemLink("player", inv) end end
	if not hotItemIdentifier then hotItemIdentifier = GetContainerItemLink(bagID, slotID) end
	HoTBagUpdateUpgradeArrow(slot, hotItemIdentifier)
	SetItemButtonTexture(slot, texture)
	SetItemButtonCount(slot, count)
	SetItemButtonDesaturated(slot, slot.locked or slot.junkDesaturate)

	if GameTooltip:IsOwned(slot) then
		HoTBagAppendUpgradeTooltip(slot, hotItemIdentifier)
	elseif GameTooltip:GetOwner() == slot and not slot.hasItem then
		GameTooltip_Hide()
	end
end
		...
		if numSlots > 0 then
			...
			for slotID = 1, numSlots do
				...
				if not f.Bags[bagID][slotID] then
					...
					HoTBagEnsureUpgradeArrow(f.Bags[bagID][slotID])
					if not f.Bags[bagID][slotID].__hotTooltipHooked then
						f.Bags[bagID][slotID]:HookScript("OnEnter", function(button)
							local itemIdentifier
							if button.GetInventorySlot then local inv = button:GetInventorySlot(); if inv then itemIdentifier = GetInventoryItemLink("player", inv) end end
							if not itemIdentifier then itemIdentifier = GetContainerItemLink(bagID, slotID) end
							HoTBagAppendUpgradeTooltip(button, itemIdentifier)
						end)
						f.Bags[bagID][slotID].__hotTooltipHooked = true
					end
