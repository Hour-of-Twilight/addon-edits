...
local function HoT_UpdateUFLevel(frame, unit)
	...
	local level = HoT_GetUnitProgressionLevel(unit)
	if level then
...
local function HoT_RefreshUFProgression()
	HoT_UpdateUFLevel(_G.ElvUF_Player, "player")
	HoT_UpdateUFLevel(_G.ElvUF_Target, "target")
	HoT_UpdateUFLevel(_G.ElvUF_Focus, "focus")
	HoT_UpdateUFLevel(_G.ElvUF_TargetTarget, "targettarget")
end
...
function UF:Construct_UF(frame, unit)
	...
	-- ToxiUI added shadow for frames
	local edgeSize, shadowSize, shadowAlpha = 1, 3, 1;
	local softShadow = frame.txSoftShadow or CreateFrame("Frame", "Shadow", frame)
...
function UF:GetAuraAnchorFrame(frame, attachTo, isConflict)
	if isConflict then
		E:Print(format(
			L
			["%s frame(s) has a conflicting anchor point, please change either the buff or debuff anchor point so they are not attached to each other. Forcing the debuffs to be attached to the main unitframe until fixed."],
...
function UF:UpdateColors()
	...
	ElvUF.colors.DebuffHighlight.Disease = E:SetColorTable(ElvUF.colors.DebuffHighlight.Disease,
		db.debuffHighlight.Disease)
...
function UF.groupPrototype:Configure_Groups(frame)
	...
	local xMult, yMult = DIRECTION_TO_HORIZONTAL_SPACING_MULTIPLIER[direction],
		DIRECTION_TO_VERTICAL_SPACING_MULTIPLIER[direction]
		...
		if (i - 1) % db.groupsPerRowCol == 0 then
			...
			if DIRECTION_TO_POINT[direction] == "LEFT" or DIRECTION_TO_POINT[direction] == "RIGHT" then
				if newRows == 1 then
					...
					newCols = newCols + 1
				elseif group then
					group:Point(point, frame, point,
						((((db.width + db.horizontalSpacing) * 5) * ((i - 1) % db.groupsPerRowCol)) + ((i - 1) % db.groupsPerRowCol) * groupSpacing) *
						xMult, (((UNIT_HEIGHT + db.verticalSpacing + groupSpacing) * (newRows - 1))) * yMult)
				end
			else
				if newCols == 1 then
					if group then
				...
				elseif group then
					group:Point(point, frame, point,
						...
						((((UNIT_HEIGHT + db.verticalSpacing) * 5) * ((i - 1) % db.groupsPerRowCol)) + ((i - 1) % db.groupsPerRowCol) * groupSpacing) *
						yMult)
				end
			end
		end
	...
	if numGroups then
		if db.raidWideSorting then
			if not self[group].groups[1] then
				self[group].groups[1] = self:CreateHeader(self[group], nil,
					"ElvUF_" .. E:StringTitle(self[group].groupName) .. "Group1", template or self[group].template, nil,
					headerTemplate or self[group].headerTemplate)
			end
		else
			while numGroups > #self[group].groups do
				local index = tostring(#self[group].groups + 1)
				tinsert(self[group].groups,
					self:CreateHeader(self[group], index, "ElvUF_" ..
						E:StringTitle(self[group].groupName) .. "Group" .. index, template or self[group].template, nil,
						headerTemplate or self[group].headerTemplate))
			end
		end
		...
		UF.headerFunctions[group].Update = function()
			...
			for i = 1, UF[group]:GetNumChildren() do
				...
				if _G[child:GetName() .. "Target"] then
					UF["Update_" .. E:StringTitle(group) .. "Frames"](UF, _G[child:GetName() .. "Target"],
						UF.db.units[group])
				end

				if _G[child:GetName() .. "Pet"] then
					UF["Update_" .. E:StringTitle(group) .. "Frames"](UF, _G[child:GetName() .. "Pet"],
						UF.db.units[group])
				end
			end
...
function UF:Initialize()
	...
	if E.private.unitframe.disabledBlizzardFrames.target then
		...
		InterfaceOptionsCombatPanelEnemyCastBarsOnNameplates:Point(InterfaceOptionsCombatPanelEnemyCastBarsOnPortrait
			:GetPoint())
...
-- HoT progression overlays / cache requests
local HoTUFHookFrame = CreateFrame("Frame")
HoTUFHookFrame:RegisterEvent("PLAYER_ENTERING_WORLD")
HoTUFHookFrame:RegisterEvent("PLAYER_TARGET_CHANGED")
HoTUFHookFrame:RegisterEvent("PLAYER_FOCUS_CHANGED")
HoTUFHookFrame:RegisterEvent("UNIT_INVENTORY_CHANGED")
HoTUFHookFrame:RegisterEvent("PLAYER_EQUIPMENT_CHANGED")
HoTUFHookFrame:SetScript("OnEvent", function()
	HoT_RefreshUFProgression()
end)

C_Timer.After(1, HoT_RefreshUFProgression)
C_Timer.After(3, function()
	if type(UpdateInjectorLevel) == "function" then
		hooksecurefunc("UpdateInjectorLevel", function()
			HoT_RefreshUFProgression()
		end)
	end
end)
