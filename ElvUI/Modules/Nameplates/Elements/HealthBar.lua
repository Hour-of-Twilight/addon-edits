		...
		if (not status) or (status and not NP.db.threat.useThreatColor) then
			...
			elseif reactionType and reactionType > 4 then
				if frame.UnitType == "FRIENDLY_PLAYER" then
					r, g, b = db.reactions.friendlyPlayer.r, db.reactions.friendlyPlayer.g, db.reactions.friendlyPlayer
					.b
				else
...
function NP:Update_Health(frame)
	...
	if self.db.units[frame.UnitType].health.enable or (frame.isTarget and self.db.alwaysShowTargetHealth) and self.db.units[frame.UnitType].health.text.enable and hpFont then
		local currentFont = frame.Health.Text:GetFont()
		if not currentFont and hpFont then
			frame.Health.Text:SetFont(hpFont, self.db.units[frame.UnitType].health.text.fontSize, self.db.units[frame.UnitType].health.text.fontOutline)
		end
...
function NP:Configure_HealthBar(frame, configuring)
	...
	if configuring then
		...
		if db.text.enable then
			...
			healthBar.Text:Point(E.InversePoints[db.text.position],
				db.text.parent == "Nameplate" and frame or frame[db.text.parent], db.text.position, db.text.xOffset,
				db.text.yOffset)
