...
local ServerClassTokenMap = {
    ...
    [17] = "HUNTER",      -- Ranger fallback
    [18] = "SHAMAN",      -- Savage fallback
}

E.ServerClassDisplayMap = ServerClassDisplayMap
E.ServerClassTokenMap = ServerClassTokenMap

function E:GetServerClassInfo(unit)
    ...
    return displayName, classToken, subclassID
end

function E:GetServerClassToken(unit)
    local _, classToken = self:GetServerClassInfo(unit)
    return classToken
end

function E:GetServerClassName(unit)
    local displayName = self:GetServerClassInfo(unit)
    return displayName
end

function E:GetServerClassID(unit)
    local _, _, subclassID = self:GetServerClassInfo(unit)
    return subclassID
end

function E:GetServerClassColor(unit)
    local classToken = self:GetServerClassToken(unit)
    return classToken and ((CUSTOM_CLASS_COLORS and CUSTOM_CLASS_COLORS[classToken]) or RAID_CLASS_COLORS[classToken])
end

function E:FormatUnitNameForCache(unit)
    local name = UnitName(unit)
...
function E:GetServerClassTag(unit)
    local displayName, _, subclassID = self:GetServerClassInfo(unit)
    if subclassID ~= nil then
        return displayName, subclassID
    end
...
function E:RefreshMyServerClass()
    ...
    E.myServerClassName, E.myServerClassToken, E.myServerClassID = E:GetServerClassInfo("player")
end

function E:PLAYER_LOGIN()
    self:RefreshMyServerClass()
end
...
function E:CheckRole(event)
	...
	local role
	local classRole = self.ClassRole and self.ClassRole[self.myclass]

	if type(classRole) == "string" then
		role = classRole
	elseif type(classRole) == "table" and talentTree then
	...
	if E.myclass == "SHAMAN" and self.DispelClasses and self.DispelClasses[self.myclass] then
